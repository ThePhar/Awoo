...
export default class Player {
  ...
  public async accuse(accusing: string): Promise<boolean> {
    ...
    if (accused.length > 1) {
      await this.game.announce(dedent(`
    ...
    if (this.accusing) {
      ...
      if (!this.game.allowVoteChange) {
        await this.game.announce(`${this}, you cannot change your vote at this stage of the game. All votes are final.`)
        return false
      }
    }
  ...
  public async clearAccusationUserDriven(): Promise<void> {
    ...
    // Don't allow vote change if not allowed to change.
    if (!this.game.allowVoteChange) {
      await this.game.announce(`${this}, you cannot clear your vote at this stage of the game. All votes are final.`)
      return
    }
