...
export default class Player {
  ...
  accuse(accusing: string): boolean {
    // Player is dead.
    if (!this.alive) {
      this.game.announce(AccusationTemplate.ghostLynch(this));
      return false;
    }
    ...
    if (this.game.phase !== Phase.Day) {
      this.game.announce(AccusationTemplate.nonDayLynch(this));
      return false;
    }

    // Player is targeting themselves.
    if (accusedPlayer.id === this.id) {
      this.game.announce(AccusationTemplate.selfLynch(this));
      return false;
    }

    // Accusing player is dead.
    if (!accusedPlayer.alive) {
      this.game.announce(AccusationTemplate.deadLynch(this));
      return false;
    }

    // All else is good!
    this.accusing = accusedPlayer;
    this.game.announce(AccusationTemplate.success(this, accusedPlayer));
    return true;
  }
